  ...
  if (WAL_RCV_USE_ERPC) {
    ...
    if (wrcvstatus)
      ...
      if (WAL_RCV_USE_ERPC) {
        ...
        if (!WAL_RCV_USE_ERPC || wal_rcv_erpc_server == (void *)NULL)
          ...
          ereport(LOG, (errmsg("endofwal, breaking streaming loop\n")));
				...
				//ereport(LOG, (errmsg("assertion waitfd != PGINVALID_SOCKET passed\n")));
        /*
        ...
        */
				//ereport(LOG, (errmsg("WaitLatchOrSocket is giving error")));
        /*
				ereport(LOG, (errmsg("Sleeping after streaming")));
        sleep(1);
				ereport(LOG, (errmsg("Wake me up inside (cant wake up)")));
        */
        if (!WAL_RCV_USE_ERPC && (rc & WL_LATCH_SET))
				{
          ereport(LOG, (errmsg("entering WaitLatchOrSocket walreceiver 596")));
				...
				if (!WAL_RCV_USE_ERPC && (rc & WL_TIMEOUT))
				{
					/*
    ...
    if (WAL_RCV_USE_ERPC && wal_rcv_erpc_client != (void *)NULL) {
      // destroy eRPC client
